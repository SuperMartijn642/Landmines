...
public class LandmineRenderer implements CustomBlockEntityRenderer<LandmineBlockEntity,LandmineRenderer.State> {
    ...
    public void updateState(State state, LandmineBlockEntity entity, UpdateContext context){
        ...
        List<BlockStateModelPart> parts = state.blockRenderState.setupModel(IDENTITY_MATRIX, model.hasMaterialFlag(level, pos, visualState, BakedQuad.FLAG_TRANSLUCENT));
        random.setSeed(seed);
        model.collectParts(level, pos, visualState, random, parts);
