    ...
    LandmineEffect POTION = (level, pos, stack) -> {
        if(!level.isClientSide){
            ...
            if(isWater){ // water potion
                ...
                List<LivingEntity> entities = level.getEntitiesOfClass(LivingEntity.class, area, entity -> entity instanceof EndermanEntity || entity instanceof BlazeEntity);
        ...
        if(!level.isClientSide){
            ...
            level.getEntitiesOfClass(LivingEntity.class, new AxisAlignedBB(pos).inflate(0.7))
                .forEach(entity -> {
                    ...
                    for(int i = 0; i < 16; ++i){
                        double teleportX = entity.position().x + (level.getRandom().nextDouble() - 0.5) * 2 * range;
                        double teleportY = MathHelper.clamp(entity.position().y + (level.getRandom().nextDouble() - 0.5) * 2 * range, 0, level.getHeight() - 1);
                        double teleportZ = entity.position().z + (level.getRandom().nextDouble() - 0.5) * 2 * range;
